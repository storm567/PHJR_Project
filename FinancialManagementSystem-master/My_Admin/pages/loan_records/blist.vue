          ...
          <uni-tr v-for="(item,index) in filteredRecords" :key="index">
            ...
            <uni-td align="center">
              <view class="uni-group">
				...
				<button v-if="item.loan_state == 1" @click="navigateTo('./approval?id='+item._id, false)" class="uni-button" size="mini" type="default">审批</button>
              </view>
            </uni-td>
          </uni-tr>
        </uni-table>
    ...
    methods: {
		...
		filterRecords() {
		    ...
		    this.filteredRecords = this.records.filter(item => {
		      ...
		      return ['1', '3', '4'].includes(state); // 使用字符串进行比较
		    });
